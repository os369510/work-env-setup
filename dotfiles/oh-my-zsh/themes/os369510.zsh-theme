# this is forked from oh-my-zsh - jonathan.zsh-theme
function theme_precmd {
  ...
  local instancesize=$(($(get_instance | wc -c) + 3 )) # - '\n' + '[]--'
  local promptsize=$((${#${(%):---(%n@%m:%l)---()--}} + instancesize))
...
${PR_RED}%{$(get_instance)%}${PR_YELLOW}]\
${PR_CYAN}${PR_HBAR}${PR_HBAR}${PR_GREY}(\
${PR_GREEN}%${PR_PWDLEN}<...<%~%<<\
